...
As of 2026-05-15, the current Atlas checkout root (`<atlas_root>`) is the canonical Codex-native base of ATLAS.
...
- Canonical source of truth moved out of REYESOFT and established in the Atlas repository root (`<atlas_root>`)
...
- `surface-audit`: documented and executable for Atlas root coherence between README and canonical surface
...
- `market-research-benchmark` can return a structured benchmark report over local reference evidence and documented radar repos
...
- Atlas now exists as its own git repository and canonical GitHub remote
...
- Atlas still does not mutate the user Codex config (`~/.codex/config.toml`) or add a project-scoped `.codex/config.toml`; runtime compatibility is observed, not changed
...
- Atlas now keeps component-inspiration readiness separate from actual implementation: 21st/Context7 presence, derivative risk, approval needs and local fallbacks are explicit before any future UI-pattern use
...
- Deprecated flat skill compatibility pointers have already been removed from the canonical tracked surface
- The canonical tracked surface no longer includes the previously blocked `_legacy_flat` or bootstrap-residue directories referenced by older cleanup notes
