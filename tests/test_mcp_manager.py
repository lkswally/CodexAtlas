...
def test_list_mcp_profiles_exposes_docs_search_as_only_experimental_profile():
    ...
    magic = next(item for item in result["profiles"] if item["id"] == "21st_magic")
    ...
    assert magic["experimental_enabled"] is False
    assert magic["lifecycle_state"] == "blocked"
...
def test_watchlist_or_deferred_mcps_stay_blocked():
    ...
    magic_approval = approve_mcp("21st_magic", "Trying to approve Magic MCP should fail while watchlisted.")
    ...
    assert magic_approval["state"] == "blocked"
    assert "mcp_profile_not_approved_for_experiment:21st_magic" in magic_approval["blockers"]
