...
def _bootstrap_like_project(project_root: Path, project_profile: str, extra_dirs: list[str]) -> None:
    metadata = {
        "schema_version": "1.0",
...
def test_certify_project_passes_clean_backend_service():
    ...
    try:
        ...
        _bootstrap_like_project(project_root, "backend_service", ["api", "services", "tests"])
...
def test_certify_project_reports_blockers_for_atlas_and_claude_residue():
    ...
    try:
        ...
        _bootstrap_like_project(project_root, "ai_agent_system", ["agents", "skills", "evaluations", "prompts"])
