...
Stability retry on 2026-06-15 used the same fixture, Python 3.13.14 `.venv`, local Playwright Chromium, explicit `ATLAS_RUN_REAL_BROWSER_SMOKE=1`, and separate temp directories outside the repo. Two fresh runs completed successfully: run #1 reported `1 passed in 29.52s` with an external measured duration of `31.15s`; run #2 reported `1 passed in 2.98s` with an external measured duration of `3.49s`. Both runs produced `desktop.png` at 481026 bytes, `mobile.png` at 131727 bytes, two viewport reports, zero console errors, zero network errors, PASS bundle validation, and PASS non-blocking Evidence Quality Report output. No workflow integration was added; the browser smoke remains manual/opt-in.

Manual GitHub Actions workflow `Evidence Browser Smoke` was added and observed on 2026-06-17. The final successful run `27724893071` used `windows-2025`, Python 3.13, installed Playwright Chromium, executed the real-browser smoke with `1 passed in 3.03s`, executed the Evidence baseline with `69 passed in 0.31s`, and uploaded artifact `evidence-browser-smoke` ID `7709863742` with size `1212140` bytes. Earlier attempts exposed two workflow-only issues before the successful run: a missing parent directory for `--basetemp` on a fresh checkout, and an artifact path under a hidden directory that `upload-artifact` did not package. The final workflow writes smoke outputs directly to non-hidden `evidence-browser-smoke-artifact` and remains manual/opt-in, outside Atlas CI, with no PR or push trigger.
